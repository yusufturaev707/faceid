...
class Supervisor(BaseModel):
    ...
    role = models.CharField('Rol', max_length=20, choices=ROLE_CHOICES, default='supervisor')
...
class EventSupervisor(BaseModel):
    supervisor = models.ForeignKey('supervisor.Supervisor', on_delete=models.SET_NULL, blank=True, null=True, verbose_name=_('Nazoratchi'))
    ...
    access_datetime = models.DateTimeField(verbose_name=_("Boshlanish vaqti"))
    expired_datetime = models.DateTimeField(verbose_name=_("Tugash vaqti"))
    is_participated = models.BooleanField(default=False, verbose_name=_("Davomat"))

    def __str__(self):
        return f"{self.supervisor.fio if self.supervisor else ''} - {self.exam.test.name}|{self.exam.start_date}"
